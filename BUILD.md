...
2. 點 **「匯入」** ➔ 上傳 `Day1.csv`
3. 「**選擇要放置地標的欄位**」：勾選 **`Latitude`** 和 **`Longitude`** 兩欄
   - 🚨 **座標欄名必須是英文**。My Maps 是靠欄名判斷這是不是座標；欄名寫中文「緯度／經度」時，它會把數值當成地址去做地理編碼，然後回報「**糟糕！我們找不到這些地點。您選擇的地點欄正確嗎？**」。`export_mymaps.py` 已固定輸出英文欄名並排在最前兩欄。
4. 「**選擇要做為地標標題的欄位**」：選 **`名稱`**
5. 圖層名稱改成「Day 1」
...
- ⚠️ **My Maps 完全不吃 Place ID**，只認經緯度或地理編碼。因此 CSV 的定位依據是 `Latitude`／`Longitude` 欄，與行程表導航連結用的 `query_place_id` 是兩套機制。
