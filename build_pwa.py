...
# 每日概況：顯示在該天時間軸最上方的一小塊。
#
# 收錄原則——**只放「會改變出門前準備」的資訊**。
#   ✅ 天氣（決定帶不帶雨具、穿什麼）
#   ✅ 步行量（決定穿球鞋還是涼鞋）——這項每天都有
#   ✅ 該天獨有的準備（現金、購物袋、行動電源…）
#   ❌ 「正午在室內」「護照放手邊」這類不影響準備的敘述，一律不寫
#
# ⚠️ 天氣是時效性資訊，出發前請重新查詢並更新這裡（只需改這個 dict 再重新編譯）。
#    資料來源：日本氣象廳 https://www.jma.go.jp/bosai/forecast/data/forecast/130000.json
WEATHER_UPDATED = '2026-08-21 17:00 發布（日本氣象廳）'
DAY_BRIEF = {
    1: {'weather': '8/20（四）<strong>晴、傍晚轉多雲</strong>　最高 33°C　降水 0–20%',
        'walk': '<strong>少</strong>——大半時間在飛機與電車上，秋葉原只逛約 1 小時。<strong>涼鞋可</strong>。',
        'extra': []},
    2: {'weather': '8/21（五）多雲、午後局部雷雨　24–32°C　降水 30%（<strong>今日已過</strong>）',
        'walk': '長輩組 <strong>多</strong>／親子組 <strong>最多</strong>。',
        'extra': []},
    3: {'weather': '8/22（六）多雲下午轉雷雨，入夜有雨　24–31°C　<strong>降水 50%</strong>',
        'walk': '<strong>重度</strong>，國立科學博物館站立時間長，下午阿美橫丁步行。<strong>穿好走的鞋</strong>。',
        'extra': ['☔ <strong>雨量預估降至 0.4mm，但仍防局部強雷雨</strong>：數值模式預測的降雨訊號減弱（下午 12:00–19:00 逐時雨量降為 0.0mm），'
                  '但<strong>氣象廳仍維持強雷雨警告</strong>。下午的阿美橫丁沒有拱廊，<strong>雨傘雨衣仍為必備</strong>；'
    ...
    4: {'weather': '8/23（日）多雲，早晨局部微雨　23–34°C　降水 40%',
        'walk': '<strong>中等</strong>（吉卜力美術館＋吉祥寺商圈與橫丁）。<strong>穿好走的鞋</strong>。',
        'extra': ['🏠 <strong>雨量預估僅 0.6mm，這天不太怕雨</strong>：'
                  '早上可能有短暫微雨（0.1mm），下午起無雨。'
                  '吉卜力美術館在室內、下午的哈莫尼卡橫丁與 Sunroad 都有拱廊頂棚，'
                  '真正會淋到的只有建築之間那幾十公尺。']},
    5: {'weather': '8/24（一）多雲時晴，高溫炎熱　25–35°C　降水 30%',
        'walk': '<strong>中等偏多</strong>（淺草寺參道＋晴空街道＋新宿）。<strong>穿好走的鞋</strong>。',
        'extra': ['🌇 <strong>看夜景條件極佳</strong>：新宿 18–21 時降水 <strong>1~4%</strong>、'
                  '<strong>雲量僅 1~2%</strong>、能見度 17 公里，都廳 Plan A 照走。',
...
def render_day_brief(day):
    """該天時間軸最上方的概況區塊。內容見 DAY_BRIEF 的收錄原則。"""
    b = DAY_BRIEF.get(day)
    if not b:
        return ''
    extra = ''.join(f'<div class="brief-row">{x}</div>' for x in b['extra'])
    return f"""      <div class="day-brief">
        <div class="brief-row brief-weather">🌤️ {b['weather']}</div>
        <div class="brief-row">🚶 <strong>步行量</strong>：{b['walk']}</div>
{extra}        <div class="brief-note">天氣為 {WEATHER_UPDATED}，出發前請再確認</div>
      </div>
"""
...
<head>
  ...
  <style>
    ...
    * {{
    ...
    /* 每日概況：只放會改變出門前準備的資訊（天氣、步行量、當天特有的攜帶物） */
    .day-brief {{
      margin: 0 0 18px;
      padding: 13px 15px;
      border-radius: 14px;
...
{timeline_html}
  ...
  <script>
    ...
    // 全部打勾或找不到時退回置頂。
    //
    // ⚠️ 但「那天一項都還沒勾」時一律停在頂部：那代表還沒開始這一天，
    //    此時該先看到當日概況（天氣、步行量、要帶什麼）、當天主題與方案切換鈕。
    //    無條件自動捲會把這三樣全部推出畫面，還要往回滑才看得到。
    //    只有已經勾過幾項（行程進行中）才跳到下一步，那才是這功能有用的時機。
    function scrollToFirstUnchecked(day) {{
      ...
      const boxes = [...sec.querySelectorAll('.check-wrapper input[type="checkbox"]')]
        .filter(cb => cb.offsetParent !== null);
      if (!boxes.some(cb => cb.checked)) return toTop();
      const pending = boxes.find(cb => !cb.checked);
